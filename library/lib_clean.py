...
from ellipse_rig.library import lib_namespace, lib_names, lib_controlGroup
...
def nspaceCleanner(dicNspaces):
    lNspace = lib_namespace.lsAllNspace(toExclude=['UI', 'shared'])
...
def deleteNspace():
    lNspace = lib_namespace.lsAllNspace(['UI', 'shared'])
    for nspace in lNspace:
        lib_namespace.removeNspace(nspace, keep=True)
