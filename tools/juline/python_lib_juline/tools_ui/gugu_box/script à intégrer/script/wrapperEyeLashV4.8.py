def build_facial_wrapper():

	"""
	SCRIPT WRAPPER EYELASHES / EYEBROWS V4.7
	...
	DEBUG V4.7:
	ligne 33 : trouver le skinCluster avec et sans majuscule
	"""
	...
	for shp in shpsMeshBody :
		...
		if shpSets:
			for s in shpSets :
				if "skinCluster" or "SkinCluster" in s:
	...
	#skiner wrprBase
	HeadJoint = mc.ls('*:head_skn')
	mc.skinCluster(HeadJoint[0], wrprBase, toSelectedBones=True, name=wrpr + "Deformed_skinCluster1")
