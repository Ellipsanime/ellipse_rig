...
# #contact : julinebreton@gmail.com

from python_lib_juline.utils.maya_widget import MayaWidget, load_ui
import os, sys
...
from python_lib_juline.library import ctrlShapesLib as shpLib
from python_lib_juline.library import functionsLib

class BendTool(MayaWidget):

    def __init__(self, parent=None):
        MayaWidget.__init__(self, 'Bend Tool', parent=parent)
        self.ui = load_ui(os.path.join(os.path.dirname(__file__), 'bend.ui'))
        ...
        self.ui.btn_applyBend.clicked.connect(self.bend)
    ...
    def bend(self):
        ...
        mc.addAttr(ctrl, longName = "bend", attributeType = "float", minValue = -10, maxValue = 10, keyable=True)
        mc.addAttr(ctrl, longName = "highBound", attributeType = "float", minValue = 0, maxValue = 10, defaultValue = 5, keyable=True)
        mc.addAttr(ctrl, longName = "lowBound", attributeType = "float", minValue = 0, maxValue = 10, defaultValue = 5, keyable=True)
        ...
        mc.setAttr(ctrl+".bend", 10)
        mc.setAttr(bend+".curvature", 180)
        mc.setDrivenKeyframe(bend, attribute="curvature", currentDriver=ctrl+".bend")
        mc.setAttr(ctrl+".bend", -10)
        mc.setAttr(bend+".curvature", -180)
        mc.setDrivenKeyframe(bend, attribute="curvature", currentDriver=ctrl+".bend")
        mc.setAttr(ctrl+".highBound", 0)
        mc.setAttr(bend+".highBound", 0)
        mc.setDrivenKeyframe(bend, attribute="highBound", currentDriver=ctrl+".highBound")
        mc.setAttr(ctrl+".highBound", 10)
        mc.setAttr(bend+".highBound", 2)
        mc.setDrivenKeyframe(bend, attribute="highBound", currentDriver=ctrl+".highBound")
        mc.setAttr(ctrl+".lowBound", 0)
        mc.setAttr(bend+".lowBound", 0)
        mc.setDrivenKeyframe(bend, attribute="lowBound", currentDriver=ctrl+".lowBound")
        mc.setAttr(ctrl+".lowBound", 10)
        mc.setAttr(bend+".lowBound", -2)
        mc.setDrivenKeyframe(bend, attribute="lowBound", currentDriver=ctrl+".lowBound")
        # set default value
        mc.setAttr(ctrl+".bend", 0)
        mc.setAttr(ctrl+".lowBound", 5)
        mc.setAttr(ctrl+".highBound", 5)
