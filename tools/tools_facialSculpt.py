...
import glob
import re
from core import namespace as lNspace
reload(lNspace)
from library import lib_deformers, lib_pipe, lib_names,lib_namespace, lib_controlGroup, lib_blendShape
...
def getLastInputId(bs):
    lastId = mc.getAttr(bs+'.inputTarget[0].inputTargetGroup', mi=True)[-1]
    return lastId
...
def genDicTrgtIds(bs):
    ...
    i = 0
    dicTrgtIds = {}
    for lol in range(0, len(lWght)/2):
        dicValues = {}
        ...
        if chkInBet < 1:
            ...
            return
        shp = mc.listConnections(bs+'.inputTarget[0].inputTargetGroup['+dicValues['index']+'].inputTargetItem['+'6000'+'].inputGeomTarget', s=True)[0]
        if not shp in dicTrgtIds.keys():
            dicTrgtIds[shp] = {}
        dicTrgtIds[shp] = dicValues
        i += 2
    return dicTrgtIds
...
def addTrgt(lNeutral, ctrl):
    ...
    if not mc.attributeQuery(attr, n=ctrl, ex=True):
        ...
        max = mc.attributeQuery(attr, n=ctrl, max=True)[0]
        if min == 0.0:
            mc.addAttr(ctrl+'.'+attr, e=True, min=-20)
            valCtrl = -20
        if min == -20.0:
            if max == 0.0:
                mc.addAttr(ctrl+'.'+attr, e=True, max=20)
                valCtrl = 20
            elif max == 20.0:
    ...
    for neutral in lNeutral:
        ...
        if mc.objExists(nspace+':'+trgt):
            ...
            mc.setDrivenKeyframe(bs+'.'+newTrgt.split(':')[-1], cd=ctrl+'.'+attr, itt='linear', ott='linear', dv=0, v=0)
            mc.setDrivenKeyframe(bs+'.'+newTrgt.split(':')[-1], cd=ctrl+'.'+attr, itt='linear', ott='linear', dv=valCtrl, v=2)
            mc.setDrivenKeyframe(bs+'.'+newTrgt.split(':')[-1], cd=ctrl+'.'+attr, itt='linear', ott='linear', dv=0, v=0)
            mc.setDrivenKeyframe(bs+'.'+newTrgt.split(':')[-1], cd=ctrl+'.'+attr, itt='linear', ott='linear', dv=valCtrl, v=2)
...
def ctrMixCombination_v2(lMshResult):
    #TYPES :__________#
    # 0 = target      #
    # 1 = corrective  #
    # 2 = mixTrgt     #
    ...
    for mshResult in lMshResult:
        ...
        for trgr in lTrgr:
            ...
            nTrgr = nTrgr.split('.')[-1][len(nTrgr.split('.')[-1].split('_')[0])+1 :].replace('_', '')
...
def connectMixShp_v3(lMixSculpt):
    for mixSculpt in lMixSculpt:
        shpEdit = False
...
def mirrorMix(lMixSculpt, SrcSide):
    mSide = 'R'
    if side == mSide:
        mSide = 'L'
    for mixSculpt in lMixSculpt:
        lIds = m.getAttr(mixSculpt+'.trgrList', m=True) or []
        if lIds:
            for id in lIds:
...
def connectResult(msh, type, conTrgt=True):
    ...
    if not bsRes:
        ...
        skin = mc.ls(mc.listHistory(result, ac=True), type='skinCluster') or []
        if skin:
            skin = skin[0]
...
def linkTargets_v2(ctrl, lNeutral):
...
def getDicBsTrgtDatasAsAlias(bs):
    ...
    for id in range(0, len(lWght)/2):
        ...
        lInputs = mc.getAttr(bs+'.'+trgtAttr, mi=True)
        for input in lInputs:
            #print bs+'.'+trgtAttr+'['+str(input)+'].inputGeomTarget'
            mshTrgt = mc.listConnections(bs+'.'+trgtAttr+'['+str(input)+'].inputGeomTarget', s=True)[0]
...
def addInbet_v2(mshReult, bsType):
    if bsType == 'TRGT':
        type = 0
    if bsType == 'COR':
        type = 1
        ...
        if not len(lActivedTrgt) == 1:
            ...
            if val in dicBsData[trgt]['sculpt'].keys():
                ...
                if bsType == 'TRGT':
                    ...
                    trgtInBet = genCorSculpt([mshReult], inBetVal=str(val).replace('.', ''))[0]
                    mc.addAttr(trgtInBet, ln='isInBet', at='bool', dv=True)
...
def genCorSculpt(lMshResult, inBetVal=''):
    lCtrl = []
    dicCtrlPos = {}
    ...
    if len(dicCtrlPos.keys()) > 1:
        mc.warning('you can add correctives targets only on ONE CONTROLER at a time (one corrective for one controler channel)')
        return
    elif len(dicCtrlPos[dicCtrlPos.keys()[0]]) > 1:
        mc.warning('you can add correctives targets only on ONE CHANNEL at a time (one corrective for one controler channel)')
        return
    lCorSculpt = []
    for mshResult in lMshResult:
...
def connectCorrective(lCor):
    ...
    for cor in lCor:
        ...
        skin = mc.ls(mc.listHistory(mshSrc, ac=True, f=True), type='skinCluster') or []
...
def genFlipCor(lSculptCor, side):
...
def extractShapes_trgt(lMshResults):
    lExpShp = []
    lExpShpAdds = []
                        ...
                        for trgt in dicZoneTrgt[key]:
                            ...
                            if mc.attributeQuery('ctrlChan', n=trgtSculpt, ex=True):
                                ...
                                for sep in dicZonedSep[key]:
                                    ...
                                    if mc.polyCompare(mshRes, sep) == 1:
                                        pole = ''
    ...
    for mshRes in lMshResults:
        ...
        for trgt in aliasData.keys():
            ...
            for wght in aliasData[trgt]['sculpt'].keys():
                ...
                if id != '6000':
                    dicShp['cor'+nResMsh+'_'+nBase+'6000'].append(extShp)
...
def extractShapes_mixTrgt(lMshResults):
    #getBsDef(obj, type)
    #TYPES :__________#
    # 0 = target      #
    # 1 = corrective  #
        ...
        for bs in lBs:
            ...
            if not dataBsRes == None:
                for wght in dataBsRes.keys():
                    ...
                    if wght == neutralGeo:
                         mc.setAttr(bsRes+'.'+alias, True)
                         print 'alias keeped :', alias
                    else:
                        mc.setAttr(bsRes+'.'+alias, False)
                lHist = mc.listHistory(neutralGeo, ac=True)
                skin = mc.ls(lHist, type='skinCluster')or []
                if skin:
                    mc.setAttr(skin[0]+'.envelope', 0)
                    geoRes = mc.skinCluster(skin[0], q=True, g=True)[0]

                aliasData = getDicBsTrgtDatasAsAlias(bs)
                #neutralGeo = mc.listRelatives(mc.blendShape(bs, q=True, g=True)[0], p=True)[0]

                dicZone = trgtNames()
                dicZoneTrgt = getZonedTrgt(bs)
                dicZonedSep = getSepExtractor(bs)
                resultTypeMesh = mc.blendShape(bs, q=True, g=True)[0]
                nspace = lib_namespace.getNspaceFromObj(resultTypeMesh)
                lMovedVtx = []
                dicNeutGeoVtx = {}
                dataBsMix = getDicBsTrgtDatasAsMsh(bs)
                print bs, dataBsMix
                return
...
def extractShapes_v3(lMshResults):
    if not lMshResults:
        mc.warning('pease, select the msh_results you want to extract')
        return
    #lib_pipe.saveNewRev()
...
def buildShapesTRGTRig(lMshResults):
    ...
    for mshOrig in lMshOrig:
        connectFaceRig_v3(mshOrig, conCor=True, conAdds=False, conSliceFather=True)
...
def buildFaceTpl_v2(lMshDest, doPath=True):
    lZones = []
    if lMshDest:
        mshPath = lMshDest[0]
        if doPath == True:
            ...
            if mc.getAttr(sculpt+'.extractedAs') == 'TRGT':
                if not mc.attributeQuery('isInBet', n=sculpt, ex=True):
                    if mc.attributeQuery('ctrlChan', n=sculpt, ex=True) and mc.getAttr(sculpt+'.ctrlChan') != '//':
                        ...
                        if mc.polyCompare(sculptOrig, mshDest) == 0:
...
def connectFaceRig_v3(mshDest, conCor=True, conAdds=True, conSliceFather=True):
    dicNeutGeoVtx = {}
    roof = 0.0
    dicZones = {}
    dicSym = {}
            ...
            if mc.getAttr(sculpt+'.extractedAs') == 'TRGT':
                if not mc.attributeQuery('isInBet', n=sculpt, ex=True):
                    if mc.attributeQuery('ctrlChan', n=sculpt, ex=True) and mc.getAttr(sculpt+'.ctrlChan') != '//':
                        activeDef(mc.listRelatives(sculpt, s=True, ni=True)[0], False)
                        sculptOrig = mc.listRelatives(sculpt, s=True, ni=False)[-1]
                        if mc.polyCompare(sculptOrig, mshDest) in [0, 32]:
                            if mc.attributeQuery('sepZone', n=sculpt, ex=True):
                                zone = mc.getAttr(sculpt+'.sepZone', asString=True)
                                if not zone in dicZones.keys():
                                    dicZones[zone] = {}
                            ...
                            if not chkConnTrgt:
                                ...
                                if not sculpt.split(':')[-1] in mc.blendShape(bs, q=True, t=True):
                                    id = getAttrIds(bs, 'weight')[-1]+1
                                    mc.blendShape(bs, edit=True, t=(mshDest, id, sculpt, 1.0))
                        else:
                            print '#####################################'
...
def smfaddTarget_ui(lNeutral):
    ...
    mc.setParent('..')
    mc.button(label='add', c='tools_facialShapes_v3.addTrgt(mc.ls(sl=True), "c_targets")')
...
def smfBlendShapesTool_ui():
    ...
    mc.menu(l='Tools MOD')
    mc.menuItem(l='dag menu', c='tools_facialShapes_v3.launchDagHack()')
    mc.menuItem(l='display handles', c='tools_facialShapes_v3.displayHandles(mc.ls(sl=True))')
    mc.separator(h=2)
    mc.setParent('..')
    mc.menu(l='Tools Dev')
    mc.menuItem(l='crt symTab', c='tools_facialShapes_v3.crtSymAttr(mc.ls(sl=True))')
    mc.menuItem(l='normalize sep sel', c='tools_facialShapes_v3.normalizeSelSep(mc.ls(sl=True))')
    mc.menuItem(l='connect mshSrc', c='tools_facialShapes_v3.linkMshSrc(mc.ls(sl=True))')
    mc.menuItem(l='ad extractMe', c='tools_facialShapes_v3.addExtractMeAttr(mc.ls(sl=True))')
    mc.menuItem(l='presset chan drivers', c='tools_facialShapes_v3.resetChans(mc.ls(sl=True) or [])')
    mc.menuItem(l='connect parent ctrl', c='tools_facialShapes_v3.connectFatherSlicesCtrl()')
    mc.menuItem(l='link resMsh to slices', c='tools_facialShapes_v3.linkSlicesMshOrig(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='LOAD MOD    ', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.loadMOD()')
    #mc.button(l='LOAD FACE TPL', h=28, w=230, c='tools_facialShapes_v3.genSkinMsh(mc.ls(sl=True))')
    mc.button(l='UNLOAD MOD', bgc=[1,.25,.5], h=28, w=230, c='tools_facialShapes_v3.unloadMOD()')
    #
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='1. GEN NEUTRAL', h=28, w=230, c='tools_facialShapes_v3.generateNeutral(mc.ls(sl=True))')
    mc.button(l='2. GEN TRGT', h=28, w=230, c='tools_facialShapes_v3.linkTargets_v2("c_targets", mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='3. LOAD FACE TPL', h=28, w=230, c='tools_facialShapes_v3.import_face_template()')
    mc.button(l='4. BUILD RIG', h=28, w=230, c='tools_facialShapes_v3.buildTpl()')
    mc.setParent('..')
    mc.button(label='SAVE new revision', w=100, c='tools_facialShapes_v3.lib_pipe.saveNewRev()')
    mc.separator(h=10, st='in')
    #
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=1)
    mc.columnLayout(adj=True, w=200)
    mc.button(l='LOAD TARGETS', h=28, w=230, c='tools_facialShapes_v3.loadTrgt(mc.ls(sl=True))')
    mc.rowLayout(nc=2, adj=1)
    mc.textScrollList('targetsList', numberOfRows=8, ams=True, h=500, w=150, dcc='tools_facialShapes_v3.isolateTrgt()')
    mc.popupMenu(b=3, mm=False, aob=True)
    mc.menuItem(l='Isolate trgts', c='tools_facialShapes_v3.isolateTrgt()')
    mc.menuItem(l='Select trgts', c='tools_facialShapes_v3.selTrgt()')
    mc.menuItem(l='Show all trgts', c='tools_facialShapes_v3.showAllTrgt()')
    mc.menuItem(divider=True)
    mc.menuItem(l='Refresh', c='tools_facialShapes_v3.refreshTrgt()')
    ...
    mc.menuItem(l='set chan tX', c='tools_facialShapes_v3.setChannel("translateX")')
    mc.menuItem(l='set chan tY', c='tools_facialShapes_v3.setChannel("translateY")')
    mc.menuItem(l='set chan tZ', c='tools_facialShapes_v3.setChannel("translateZ")')

    mc.menuItem(l='set chan rX', c='tools_facialShapes_v3.setChannel("rotateX")')
    mc.menuItem(l='set chan rY', c='tools_facialShapes_v3.setChannel("rotateY")')
    mc.menuItem(l='set chan rZ', c='tools_facialShapes_v3.setChannel("rotateZ")')

    mc.menuItem(l='set chan sX', c='tools_facialShapes_v3.setChannel("scaleX")')
    mc.menuItem(l='set chan sY', c='tools_facialShapes_v3.setChannel("scaleY")')
    mc.menuItem(l='set chan sZ', c='tools_facialShapes_v3.setChannel("scaleZ")')

    mc.menuItem(divider=True)
    mc.menuItem(l='Invert sign (+/-)', c='tools_facialShapes_v3.invertSign()')
    mc.menuItem(l='set no chan', c='tools_facialShapes_v3.setChannel("None")')
    ##
    #mc.setParent('..')
    mc.setParent('..')
    mc.setParent('..')
    ...
    mc.button(l='CHEKER ON/OFF', h=28, w=110, c='print "toto"')
    mc.rowLayout(nc=2)
    mc.button(l='WRAP TO', h=28, w=110, c='tools_facialShapes_v3.wrapOn(mc.ls(sl=True))')
    mc.button(l='REMOVE WRAP FROM', h=28, w=110, c='tools_facialShapes_v3.removeWraps(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='ADD TRGT', h=28, w=110, c='tools_facialShapes_v3.smfaddTarget_ui(mc.ls(sl=True))')
    mc.button(l='ADD INBET', h=28, w=110, c='tools_facialShapes_v3.addInbet_v2(mc.ls(sl=True)[0], "TRGT")')


    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='REMOVE TRGT', h=28, w=110, c='tools_facialShapes_v3.removeTrgt(mc.ls(sl=True))')
    mc.button(l='REMOVE INBET', h=28, w=110, c='tools_facialShapes_v3.removeInbet(mc.ls(sl=True))')

    mc.setParent('..')
    mc.button(l='DUPPLCATE SHP', h=28, w=110, c='tools_facialShapes_v3.dupShp(mc.ls(sl=True))')
    #mc.button('switch_neutralTrgt', l='NEUTRAL/TRGT', h=28, w=110, c='tools_facialShapes_v3.showNeutral()')

    #mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='TRANSFER TO', h=28, w=110, c='tools_facialShapes_v3.snapShp(mc.ls(sl=True))')
    mc.button(l='RESET', h=28, w=110, c='tools_facialShapes_v3.resetShp(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='SET COR POSE', h=28, w=110, c='tools_facialShapes_v3.genCorSculpt(mc.ls(sl=True))')
    mc.button(l='SET INBET POSE', h=28, w=110, c='tools_facialShapes_v3.addInbet_v2(mc.ls(sl=True)[0], "COR")')
    mc.setParent('..')
    mc.button(l='CONNECT COR SCULPT', h=28, w=110, c='tools_facialShapes_v3.connectCorrective(mc.ls(sl=True))')
    mc.rowLayout(nc=2)
    mc.button(l='REMOVE COR', h=28, w=110, c='print "not available for the moment, sorryyyyy"')
    mc.button(l='REMOVE INBET', h=28, w=110, c='tools_facialShapes_v3.removeInbet(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='miror cor L to R', h=28, w=110, c='tools_facialShapes_v3.genFlipCor(mc.ls(sl=True), "L")')
    mc.button(l='miror cor R to L', h=28, w=110, c='tools_facialShapes_v3.genFlipCor(mc.ls(sl=True), "R")')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='gen mix sculpt', h=28, w=110, bgc=[.05,.2,.5], c='tools_facialShapes_v3.ctrMixCombination_v2(mc.ls(sl=True))')
    mc.button(l='connect mix shape', h=28, w=110, bgc=[.05,.05,.5], c='tools_facialShapes_v3.connectMixShp_v3(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='mirror mix L-->R', h=28, w=110, bgc=[.05,.2,.5], c='tools_facialShapes_v3.mirrorMix(mc.ls(sl=True)), "L"')
    mc.button(l='mirror mix R-->L', h=28, w=110, bgc=[.05,.2,.5], c='tools_facialShapes_v3.mirrorMix(mc.ls(sl=True)), "R"')
    ...
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='select mix sculpt', h=28, w=110, c='tools_facialShapes_v3.genMixSculpt(mc.ls(sl=True))')
    mc.button(l='select mix trgt', h=28, w=110, c='tools_facialShapes_v3.connectMixShp(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='load mix', h=28, w=110, c='tools_facialShapes_v3.loadMixValues(mc.ls(sl=True)[0])')
    mc.button(l='remove mix', h=28, w=110, c='tools_facialShapes_v3.connectMixShp(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=1)
    mc.button(l='propagate mix to', h=28, w=110, c='tools_facialShapes_v3.propagateMix(mc.ls(sl=True))')
    ###
    mc.setParent('..')
    mc.setParent('..')
    mc.separator(h=10, st='in')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='template node', h=28, w=110, c='tools_facialShapes_v3.convertToTemplate(mc.ls(sl=True))')
    mc.button(l='build rig', h=28, w=110, c='tools_facialShapes_v3.buildTpl()')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='update ctrl', h=28, w=110, c='tools_facialShapes_v3.majPosCtrl(mc.ls(sl=True))')
    mc.button(l='update tpl', h=28, w=110, c='tools_facialShapes_v3.snapTpl("cg_face")')
    ...
    mc.button(l='INIT SEP', h=28, w=110, c='tools_facialShapes_v3.genSepZones(mc.ls(sl=True)[0])')
    #mc.button(l='connect SEP', h=28, w=110, c='tools_facialShapes_v3.connectSep()')
    mc.button(l='UPDATE AUTO-WEIGHTS', h=28, w=110, c='tools_facialShapes_v3.majSepBaseWght()')
    ...
    #mc.button(l='ADD SEP', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.buildSep(mc.ls(sl=True), [mc.optionMenu("sepZone", q=True, v=True)], [mc.optionMenu("sepType", q=True, v=True)])')
    mc.button(l='ADD SLICE', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.buildSep(mc.ls(sl=True), ["None"], [mc.optionMenu("sepType", q=True, v=True)])')
    ...
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='copy wght to', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.copyBsWghtTo(mc.ls(sl=True))')
    mc.button(l='inverse wght to (make negative of)', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.invertBsWghtTo_v2(mc.ls(sl=True))')

    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    #mc.button(l='BUILD SYM MAP', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.crtSymAttr(mc.ls(sl=True))')
    mc.button(l='self flip wght', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.flipBsWght(mc.ls(sl=True))')
    mc.button(l='Normalize wght', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.normalizeSelSep(mc.ls(sl=True))')

    #mc.setParent('..')
    #mc.rowLayout(nc=2, adj=1)
    #mc.button(l='ADD WGHT TO', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.addBsWghtTo(lObj)')
    #mc.button(l='Smab', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.mirrorBsWght(mc.ls(sl=True), "R")')

    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='self mirror wght L-->R', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.mirrorBsWght(mc.ls(sl=True), "L")')
    mc.button(l='self mirror wght R-->L', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.mirrorBsWght(mc.ls(sl=True), "R")')
    ...
    mc.rowLayout(nc=2, adj=1)
    mc.button(l='create sepCheaker', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.crtSepChecker(mc.ls(sl=True))')
    #mc.button(l='delete sepCheaker', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.crtSepChecker(mc.ls(sl=True))')
    ...
    mc.button(l='wrap to', h=28, w=110, c='tools_facialShapes_v3.connectNeutralWrap(mc.ls(sl=True))')
    ...
    mc.button(l='create Tpl', bgc=[.25,.2,.0], h=28, w=230, c='tools_facialShapes_v3.genTpl(mc.ls(sl=True))')
    mc.button(l='update Tpl', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.updateTpl(mc.ls(sl=True))')
    mc.button(l='report Tpl to neutral', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v3.connTplToNeutral()')
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=2)
    mc.columnLayout()
    mc.button(l='load as driver', c='tools_facialShapes_v3.loadSelItemInList("tplDriversList")')
    ...
    mc.setParent('..')
    mc.columnLayout()
    mc.checkBoxGrp('chkBoxTr', ncb=4, l='Translate : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v3.setchkBoxAllChans("chkBoxTr")')
    mc.checkBoxGrp('chkBoxRt', ncb=4, l='Rotate : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v3.setchkBoxAllChans("chkBoxRt")')
    mc.checkBoxGrp('chkBoxSc', ncb=4, l='Scale : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v3.setchkBoxAllChans("chkBoxSc")')
    mc.setParent('..')
    mc.columnLayout()
    mc.button(l='load as driven', c='tools_facialShapes_v3.loadSelItemInList("tplDrivensList")')
    ...
    mc.setParent('..')
    mc.setParent('..')
    mc.button(l='APPLY', bgc=[.25,.2,.5], c='tools_facialShapes_v3.linkSlicesDrivers()')
    ...
    mc.rowLayout(nc=1, adj=1)
    mc.button(l='EXTRACT SHAPES', bgc=[.2,.2,.5], h=28, w=230, c='tools_facialShapes_v3.extractShapes_v3(mc.ls(sl=True))')


    mc.setParent('..')
    mc.rowLayout(nc=1, adj=1)
    mc.button(l='CONNECT SHAPES', bgc=[.2,.2,.5], h=28, w=230, c='tools_facialShapes_v3.buildShapesTRGTRig(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=3, adj=2)
    mc.button(l='template face', h=28, w=110, c='tools_facialShapes_v3.setFaceTpl()')
    mc.button(l='build face', h=28, w=110, c='tools_facialShapes_v3.buildFaceTpl_v2(mc.ls(sl=True))')
    mc.button(l='connect face', h=28, w=110, c='tools_facialShapes_v3.connectFaceRig_v3(mc.ls(sl=True)[0])')
