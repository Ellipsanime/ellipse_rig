...
from ellipse_rig.tools.dav import tools_attach, tools_stateSwitch, tools_modChr
reload(tools_attach)
reload(tools_stateSwitch)
reload(tools_modChr)

from ellipse_rig.core import debbug, scene_manager, check
reload(debbug)
...
reload(lib_dagMenu)
reload(lib_modules)
from ellipse_rig.tools.dav import tools_ctrlShape
reload(tools_ctrlShape)

from ellipse_rig.core import project_manager
reload(project_manager)

import tools_project
reload(tools_project)
mc.namespace(set=':')

#from ellipse_rig.tools.dav import tools_facialShapes_v3
#reload(tools_facialShapes_v3)
...
def launchTrgtTool():
    import tools_facialShapes_v3
    reload(tools_facialShapes_v3)
    tools_facialShapes_v3.smfBlendShapesTool_ui()

def launchTrgtTool_v2():
    import tools_facialSculpt
    reload(tools_facialSculpt)
    tools_facialSculpt.SMAB_facialSculptTool_ui()
...
def crtCenterLoc(*args, **kwargs):
    pos = lib_math.averageVectors(mc.ls(sl=True, fl=True))
    loc = mc.spaceLocator()[0]
    mc.setAttr(loc+'.translate', *pos)
...
def conIris():
    hookEyes = crtHook(['sk_eyes'])[0]

    dML = 'dM_eyeBall_L'
    if not mc.objExists(dML):
        dML = mc.createNode('decomposeMatrix', n='dM_eyeBall_L')
    mc.connectAttr('sk_eyeBall_L.worldMatrix[0]', dML+'.inputMatrix', f=True)
    ...
    mc.connectAttr('sk_eyeBall_R.worldMatrix[0]', dMR+'.inputMatrix', f=True)
...
def SMAB_rigCharacterAdd_UI():
    ...
    nProjectMenu = 'SMAB_projectMenuName'
    #nTplList = 'templates'
    #nCtrlList = 'controlers'
    if mc.window(nWin, ex=True, q=True):
        mc.deleteUI(nWin, window=True)
    win_rigCharacterAdd_UI = mc.window(nWin, t='Setup Multy Asset Builder '+version)
    ...
    initBtn = mc.menuItem(l='Init', c='tools_smab_v3.scene_manager.initWipVAni()')
    ...
    mc.setParent('..', m=True)
    mc.menuItem(divider=True, dividerLabel='LAY')
    mc.menuItem(l='build LAY', c='tools_smab_v3.scene_manager.buildLay()')

    mc.menuItem(divider=True, dividerLabel='Saves')

    mc.menuItem(optionBox=True, c='tools_smab_v3.tools_project.initSceneUI()')
    saveBtn = mc.menuItem(l='Save new revision', c='tools_smab_v3.scene_manager.saveNewRev()')
    pubBtn = mc.menuItem(l='Publish', c='tools_smab_v3.scene_manager.pubScene()')
    ...
    mc.menuItem(divider=True, dividerLabel='WIP')
    mc.menuItem(l='Chr pose tool', c='tools_smab_v3.tools_modChr.smfModChrTool_ui()')
    mc.menuItem(l='Switch tool', c='tools_smab_v3.tools_stateSwitch.stateSwitch_UI()')
    mc.menuItem(l='Asset manager', c='tools_smab_v3.tools_project.initSceneUI()')
    ...
    mc.menuItem(l='trgtTool', c='tools_smab_v3.launchTrgtTool()')
    ...
    mc.menuItem(l='trgtTool_v2', c='tools_smab_v3.launchTrgtTool_v2()')
    ...
    mc.setParent('..')
    mc.menu('debbugMenu', l='Debbug', to=True)
    mc.menuItem(divider=True, dividerLabel='03FAB')
    ...
    mc.menuItem(divider=True, dividerLabel='Build Tpl Chr Options')
    ...
    mc.menuItem(l='build', c=partial(debbug.buildModulesHp, 'chkBox_doClean', 'chkBox_doCg', 'chkBox_doMirror', 'chkBox_doVis', 'chkBox_doMatchIKFK', 'chkBox_doSave'))
    mc.menuItem(divider=True, dividerLabel='Current scene', p='debbugMenu')
    mc.menuItem(l='Reload current', c='tools_smab_v3.debbug.reopenCurrentScene()', p='debbugMenu')
    mc.menuItem(divider=True, dividerLabel='Dev', p='debbugMenu')
    mc.menuItem(l='Flush modules', c='tools_smab_v3.debbug.reloadModules("ellipse_rig")', p='debbugMenu')
    mc.menuItem(l='Tmp Shapes', c='tools_smab_v3.tools_ctrlShape.crtShapes_UI()', p='debbugMenu')
    mc.menuItem(l='Check meshs', c='tools_smab_v3.check.compareSteps("SHD:", ["RIG:"], True)', p='debbugMenu')
    #mc.menuItem(l='Tmp UI', c='tools_smab_v3.tools_project.initSceneUI()', p='debbugMenu')
    mc.menu(nProjectMenu, label='Project')
    mc.radioMenuItemCollection()
    #setProject(nChecker, nProjectMenu, nProject, prodColor, initBtn, saveBtn, pubBtn, doJsn=True,  *args)
    mc.menuItem('projSet_Smurf04', label='Smurf04', radioButton=False,
                c=partial(project_manager.setProject, nWin, nProjectMenu, 'Smurf04', [0.6, 0.75, 0.9], initBtn, saveBtn, pubBtn, 'SMAB'))

    mc.menuItem('projSet_Belfort', label='Belfort', radioButton=False,
                c=partial(project_manager.setProject, nWin, nProjectMenu, 'Belfort', [0.6, 0.75, 0.9], initBtn, saveBtn, pubBtn, 'SMAB'))

    mc.menuItem('projSet_Marsu', label='Marsu', radioButton=False,
                c=partial(project_manager.setProject, nWin, nProjectMenu, 'Marsu', [0.6, 0.75, 0.9], initBtn, saveBtn, pubBtn, 'SMAB'))

    # mc.menuItem('proj_Badjers', label='Badjers', radioButton=False, c=partial(project_manager.setProject, nWin, nProjectMenu, 'Badjers', [0.6, 0.75, 0.9]))
    # mc.menuItem('proj_Smurf', label='Lucky Luck', radioButton=False, c=partial(project_manager.setProject, nWin, nProjectMenu, 'Lucky Luck', [0.6, 0.75, 0.9]))
    ...
    mc.separator(h=5, st="in")
    mc.rowLayout(nc=2, adj=2)
    mc.button('symetryze module', h=28, w=120, c='tools_smab_v3.lib_modules.tplSym()')
    mc.button('reparent module to', h=28, w=120, c='tools_smab_v3.lib_modules.tplReparent(mc.ls(sl=True))')
    mc.setParent('..')
    ...
    mc.button(l='create loc', h=28, w=130, c='tools_smab_v3.crtCenterLoc()')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    ...
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='add keepMe', h=28, w=130, c='tools_smab_v3.lib_pipe.addFlagAttr("keepMe", 1)')
    mc.button(l='remove keepMe', h=28, w=130, c='tools_smab_v3.lib_pipe.addFlagAttr("keepMe", 0)')
    ...
    project_manager.checkProject(nWin, nProjectMenu, initBtn, saveBtn, pubBtn, 'SMAB')
