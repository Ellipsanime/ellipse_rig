...
from ellipse_rig.library import lib_controlGroup, lib_rigs, lib_namespace, lib_constraints, lib_pipe, lib_math, lib_deformers, lib_clean, lib_dagMenu
...
class cg_tree_view():
    ...
    def treePopUp(self):
        ...
        mc.menuItem(l='add sel as TPL', p=self.popUp, c='tools_smab_v2.addTplToCg(mc.ls(sl=True))')
        mc.menuItem(l='add sel as CTRL', p=self.popUp, c='tools_smab_v2.addCtrlToCg(mc.ls(sl=True))')
        mc.menuItem(divider=True)
        mc.menuItem(l='build CG', p=self.popUp, c='tools_smab_v2.buildCg()')
        mc.menuItem(divider=True)

        mc.menuItem(l='delete CG', p=self.popUp, c='tools_smab_v2.deleteSelItem(tools_smab_v2.nCgTree)')
        mc.menuItem(divider=True)
        mc.menuItem(l='parent orphans to cg_all', p=self.popUp, c='tools_smab_v2.parentOrphanCg(tools_smab_v2.nCgTree)')
        mc.menuItem(l='Select in viewport', p=self.popUp, c='tools_smab_v2.selectItemInViewport(tools_smab_v2.findCgFromTree())')
...
def SMAB_rigCharacterAdd_UI():
    ...
    mc.menuItem(l='Init wip_vanim', c='tools_smab_v2.lib_pipe.initWipVAni()')
    mc.menuItem(l='Maj refs', sm=True, to=True)
    mc.menuItem(l='ALL', c='tools_smab_v2.lib_pipe.updateRef(["MOD", "STP", "SHD", "FUR"])')
    mc.menuItem(l='MOD', c='tools_smab_v2.lib_pipe.updateRef(["MOD"])')
    mc.menuItem(l='STP', c='tools_smab_v2.lib_pipe.updateRef(["STP"])')
    mc.menuItem(l='SHD', c='tools_smab_v2.lib_pipe.updateRef(["SHD"])')
    mc.menuItem(l='FUR', c='tools_smab_v2.lib_pipe.updateRef(["FUR"])')
    mc.menuItem(l='MOD to SHD', c='tools_smab_v2.lib_pipe.switchStepRef("MOD", "high", "SHD", "base")')
    mc.menuItem(l='dVmarcket', c='tools_smab_v2.lib_pipe.publishRig(mc.file(q=True, sceneName=True), False, False, True)')
    mc.setParent('..', m=True)
    mc.menuItem(divider=True, dividerLabel='Saves')
    mc.menuItem(l='Save new revision', c='tools_smab_v2.lib_pipe.saveNewRev()')
    mc.menuItem(l='Publish', c='tools_smab_v2.lib_pipe.publishRig(mc.file(q=True, sceneName=True), False, False, False, True)')
    mc.menuItem(optionBox=True, c='tools_smab_v2.pubAndVLay()')
    mc.menuItem(divider=True, dividerLabel='Dag Menu')
    mc.menuItem(l='add expertMode', c='tools_smab_v2.addExpertMod()')
    mc.menuItem(l='hack dagMenu', c='tools_smab_v2.launchDagHack()')

    mc.setParent('..')
    mc.menu(l='Tools', to=True)
    mc.menuItem(l='connect char in pose sculpt', c='tools_smab_v2.genDifShpFromSkin(mc.ls(sl=True))')
    mc.menuItem(l='remove anime curves', c='tools_smab_v2.lib_clean.deleteAniCrv()')
    mc.menuItem(l='add vis attr', c='tools_smab_v2.lib_pipe.addVisAttr_v2(mc.ls(sl=True))')
    mc.menuItem(l='connect stateSwitchs', c='tools_smab_v2.lib_pipe.connStatesSwitch()')



    mc.setParent('..')
    mc.menu(l='Tools FACE', to=True)
    mc.menuItem(l='convert VTX sel to LOC', c='tools_smab_v2.crtLocFaceExtraTpl(mc.ls(sl=True, r=True, fl=True))')
    mc.menuItem(l='convert LOCFACE to TPL', c='tools_smab_v2.buildFaceRigAdd()')
    mc.menuItem(l='generate skinExtra mesh', c='tools_smab_v2.crtMshSkinExtra(mc.ls(sl=True))')
    mc.menuItem(divider=True, dividerLabel='Eyes tools')
    mc.menuItem(l='connect eyes', c='tools_smab_v2.connectEyes(mc.ls(sl=True))')
    mc.menuItem(l='build eyelids cock', c='tools_smab_v2.crtEyelidsTpl()')
    mc.menuItem(divider=True, dividerLabel='Eyes patch')
    mc.menuItem(l='tweak iris', c='tools_smab_v2.conIris()')
    ...
    #mc.menuItem(l='switch loc to STP', c='tools_smab_v2.lib_pipe.rebuildRefInStp("STP")')
    #mc.menuItem(l='switch loc to MOD', c='tools_smab_v2.lib_pipe.rebuildRefInStp("MOD")')

    #mc.menuItem(l='CHAR tpl', c='tools_smab_v2.GuguTplTool()')

    ######
    pan = mc.paneLayout(nPan, cn='vertical3')
    formLay = mc.formLayout(nFomLay, p=nPan, nd=100)
    ######
    ...
    mc.button(l='create cg', h=25, bgc=[0.6, 0.75, 0.9], c='tools_smab_v2.crtCg()')
    ...
    #cgTree.editTree()
    mc.button('reload_btn', p=formLay,  l='reload', h=20, c='tools_smab_v2.reloadTree(tools_smab_v2.nCgTree)')
    ...
    mc.button(l='create hook', h=28, w=130, c='tools_smab_v2.crtHook(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='surf attach', h=28, w=130, c='tools_smab_v2.tools_attach.crtSurfAttach(mc.ls(sl=True), "PATH", "SURFATTACH")')
    mc.button(l='curve attach', h=28, w=130, c='tools_smab_v2.tools_attach.crtCurveAttach(mc.ls(sl=True), "PATH", "CURVEATTACH")')
    mc.setParent('..')
    mc.setParent('..')
    ...
    mc.button(l='add spaceSwitch', h=28, w=130, c='tools_smab_v2.addSpaceSwitch("rButtonSpSw", mc.ls(sl=True))')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='remove spaceSwitch', h=28, w=130, bgc=[.5, .2, .2], c='tools_smab_v2.lib_dagMenu.removeSpaceSwitch(mc.ls(sl=True))')
    mc.button(l='remove from spaceSwitch', h=28, w=130, bgc=[.5, .2, .2], c='tools_smab_v2.lib_dagMenu.removeFromSpaceSwitch(mc.ls(sl=True))')
    mc.setParent('..')
    mc.setParent('..')

    mc.frameLayout(l='Ctrl', cll=True, bv=True, bgc=[.5, .3, .0], bgs=True)
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='shape to ctrls', h=28, w=130, c='tools_smab_v2.parentShpToCtrl(mc.ls(sl=True))')
    mc.button(l='create freeRot', h=28, w=130, c='tools_smab_v2.crtFreeRot()')
    mc.setParent('..')
    mc.button(l='link ctrl as mirror', h=28, w=130, c='tools_smab_v2.linkMirrorCtrl(mc.ls(sl=True))')
    mc.setParent('..')

    mc.frameLayout(l='Deformers', cll=True, bv=True, bgc=[.2, .3, .5], bgs=True)
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='add cluster', h=28, w=130, c='tools_smab_v2.addClst(mc.ls(sl=True))')
    mc.button(l='add softMod', h=28, w=130, c='tools_smab_v2.crtSoftMod(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='reset skin', h=28, w=130, c='tools_smab_v2.resetSkin(mc.ls(sl=True))')
    mc.button(l='hack ctrl skin', h=28, w=130, c='tools_smab_v2.lib_deformers.ctrlSkinLocal(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='sk to msh skin', h=28, w=130, c='tools_smab_v2.replaceSkByMsh(mc.ls(sl=True))')
    mc.button(l='combine msh', h=28, w=130, c='tools_smab_v2.lib_deformers.combDeformedMesh(mc.ls(sl=True))')
    mc.setParent('..')
    mc.setParent('..')
    ...
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='add deleteMe', h=28, w=130, c='tools_smab_v2.lib_pipe.addFlagAttr("deleteMe", 1)')
    mc.button(l='remove deleteMe', h=28, w=130, c='tools_smab_v2.lib_pipe.addFlagAttr("deleteMe", 0)')
    ...
    mc.menuItem(l='Load cg CTRL', c='tools_smab_v2.updateCtrlList(tools_smab_v2.nCtrlList)')
    mc.menuItem(l='Remove from cg', c='tools_smab_v2.removeFromCg(tools_smab_v2.nCtrlList)')
    mc.menuItem(divider=True)
    mc.menuItem(l='Select in viewport', c='tools_smab_v2.selectItemInViewport(tools_smab_v2.getSelItemS(tools_smab_v2.nCtrlList))')
