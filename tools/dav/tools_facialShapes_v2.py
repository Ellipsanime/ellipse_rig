...
def listOfShapes():
    ...
    dicLips['lU'] = ['li_uLower', 'li_uRaiser']
    ###########################################################
...
def trgtToCtrlChan():
    ...
    dicChansNegative['li_uRaiser'] = 'rotateZ'    #-
    dicChansPositive['li_uLower'] = 'rotateZ'     #+
...
def geStorageGrp(type, msh):
    nspace = crtNspace(msh, nsFather=type)
    nStorageGrp = 'geo_'+type.lower()
...
def ctrMixCombination_v2(lMshResult):
    for mshResult in lMshResult:
        getEditMode = False
        chkEditMode = mc.ls('*.editMode', r=True) or []
        if chkEditMode:
            getEditMode = mc.getAttr(chkEditMode)
        if getEditMode == True:
        ...
        part = mshResult[len(mshResult.split('_')[0]) :].capitalize()
        ...
        if not nMix in lComb:
            ...
            if not mc.objExists(nspace+':geo_mixSculpt'):
                mc.createNode('transform', n='geo_mixSculpt')
            mixSculpt = mc.duplicate(mshResult, n=nMixSculpt)[0]
            mc.parent(mixSculpt, nspace+':geo_mixSculpt')
            for attr in ('translate', 'rotate', 'scale'):
                for chan in ['X', 'Y', 'Z']:
                    mc.setAttr(mixSculpt+'.'+attr+chan, lock=False)
            cleanShapes(mixSculpt)
...
def connectMixShp_v3(lMixSculpt):
    for mixSculpt in lMixSculpt:
        ...
        if chkMixRes == False:
            ...
            mc.parent(mixTrgt, nspace+'geo_mixtrgt')
        elif chkMixRes == True:
            chkMixResult = mc.listConnections(mixSculpt+'.shpResult', d=True) or []
            if chkMixResult:
...
def getDicBsTrgtDatasAsAlias(bs):
    ...
    for id in range(0, len(lWght)/2):
        ...
        for input in lInputs:
            ...
            if mc.listConnections(bs+'.'+trgtAttr+'['+str(input)+'].inputGeomTarget', s=True):
                mshTrgt = mc.listConnections(bs+'.'+trgtAttr+'['+str(input)+'].inputGeomTarget', s=True)[0]
...
def extractShapes_mix(lMshResults):
    #getBsDef(obj, type)
    #TYPES :__________#
    # 0 = target      #
    # 1 = corrective  #
...
def extractShapes_v3(lMshResults):
    ...
    if lMix:
        mc.parent(lMix, 'toExport')
        lToExport.extend(lMix)
    if dicSkin:
...
def buildShapesTRGTRig(lMshResults):
    ...
    setFaceTpl()
    buildFaceTpl_v2(lMshResults, doPath=False)
    for mshRes in lMshResults:
        mshShp = initShpBuild(mshRes)
        connectFaceRig_v3(mshShp, conCor=False, conAdds=True, conSliceFather=True)
...
def connectFaceRig_v3(mshDest, conCor=True, conAdds=True, conSliceFather=False):
    dicNeutGeoVtx = {}
    roof = 0.0
    dicZones = {}
    dicSym = {}
...
def smfaddTarget_ui(lNeutral):
    ...
    mc.setParent('..')
    mc.button(label='add', c='tools_facialShapes_v2.addTrgt(mc.ls(sl=True), "c_targets")')
...
def smfBlendShapesTool_ui():
    ...
    mc.menu(l='Tools MOD')
    mc.menuItem(l='dag menu', c='tools_facialShapes_v2.launchDagHack()')
    mc.menuItem(l='display handles', c='tools_facialShapes_v2.displayHandles(mc.ls(sl=True))')
    mc.separator(h=2)
    mc.setParent('..')
    mc.menu(l='Tools Dev')
    mc.menuItem(l='crt symTab', c='tools_facialShapes_v2.crtSymAttr(mc.ls(sl=True))')
    mc.menuItem(l='normalize sep sel', c='tools_facialShapes_v2.normalizeSelSep(mc.ls(sl=True))')
    mc.menuItem(l='ad extractMe', c='tools_facialShapes_v2.addExtractMeAttr(mc.ls(sl=True))')
    mc.menuItem(l='connect mshSrc', c='tools_facialShapes_v2.linkMshSrc(mc.ls(sl=True))')
    mc.menuItem(l='presset chan drivers', c='tools_facialShapes_v2.resetChans(mc.ls(sl=True) or [])')
    mc.menuItem(l='connect parent ctrl', c='tools_facialShapes_v2.connectFatherSlicesCtrl()')
    mc.menuItem(l='link resMsh to slices', c='tools_facialShapes_v2.linkSlicesMshOrig(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='LOAD MOD    ', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.loadMOD()')
    #mc.button(l='LOAD FACE TPL', h=28, w=230, c='tools_facialShapes_v2.genSkinMsh(mc.ls(sl=True))')
    mc.button(l='UNLOAD MOD', bgc=[1,.25,.5], h=28, w=230, c='tools_facialShapes_v2.unloadMOD()')
    #
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='1. GEN NEUTRAL', h=28, w=230, c='tools_facialShapes_v2.generateNeutral(mc.ls(sl=True))')
    mc.button(l='2. GEN TRGT', h=28, w=230, c='tools_facialShapes_v2.linkTargets_v2("c_targets", mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=2)
    mc.button(l='3. LOAD FACE TPL', h=28, w=230, c='tools_facialShapes_v2.import_face_template()')
    mc.button(l='4. BUILD RIG', h=28, w=230, c='tools_facialShapes_v2.buildTpl()')
    mc.setParent('..')
    mc.button(label='SAVE new revision', w=100, c='tools_facialShapes_v2.lib_pipe.saveNewRev()')
    mc.separator(h=10, st='in')
    #
    mc.setParent('..')
    mc.rowLayout(nc=2, adj=1)
    mc.columnLayout(adj=True, w=200)
    mc.button(l='LOAD TARGETS', h=28, w=230, c='tools_facialShapes_v2.loadTrgt(mc.ls(sl=True))')
    mc.rowLayout(nc=2, adj=1)
    mc.textScrollList('targetsList', numberOfRows=8, ams=True, h=500, w=150, dcc='tools_facialShapes_v2.isolateTrgt()')
    mc.popupMenu(b=3, mm=False, aob=True)
    mc.menuItem(l='Isolate trgts', c='tools_facialShapes_v2.isolateTrgt()')
    mc.menuItem(l='Select trgts', c='tools_facialShapes_v2.selTrgt()')
    mc.menuItem(l='Show all trgts', c='tools_facialShapes_v2.showAllTrgt()')
    mc.menuItem(divider=True)
    mc.menuItem(l='Refresh', c='tools_facialShapes_v2.refreshTrgt()')
    ...
    mc.menuItem(l='set chan tX', c='tools_facialShapes_v2.setChannel("translateX")')
    mc.menuItem(l='set chan tY', c='tools_facialShapes_v2.setChannel("translateY")')
    mc.menuItem(l='set chan tZ', c='tools_facialShapes_v2.setChannel("translateZ")')

    mc.menuItem(l='set chan rX', c='tools_facialShapes_v2.setChannel("rotateX")')
    mc.menuItem(l='set chan rY', c='tools_facialShapes_v2.setChannel("rotateY")')
    mc.menuItem(l='set chan rZ', c='tools_facialShapes_v2.setChannel("rotateZ")')

    mc.menuItem(l='set chan sX', c='tools_facialShapes_v2.setChannel("scaleX")')
    mc.menuItem(l='set chan sY', c='tools_facialShapes_v2.setChannel("scaleY")')
    mc.menuItem(l='set chan sZ', c='tools_facialShapes_v2.setChannel("scaleZ")')

    mc.menuItem(divider=True)
    mc.menuItem(l='Invert sign (+/-)', c='tools_facialShapes_v2.invertSign()')
    mc.menuItem(l='set no chan', c='tools_facialShapes_v2.setChannel("None")')
    ##
    #mc.setParent('..')
    mc.setParent('..')
    mc.setParent('..')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='WRAP TO', h=28, w=110, c='tools_facialShapes_v2.wrapOn(mc.ls(sl=True))')
    mc.button(l='REMOVE WRAP FROM', h=28, w=110, c='tools_facialShapes_v2.removeWraps(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='ADD TRGT', h=28, w=110, c='tools_facialShapes_v2.smfaddTarget_ui(mc.ls(sl=True))')
    mc.button(l='ADD INBET', h=28, w=110, c='tools_facialShapes_v2.addInbet_v2(mc.ls(sl=True)[0], "TRGT")')


    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='REMOVE TRGT', h=28, w=110, c='tools_facialShapes_v2.removeTrgt(mc.ls(sl=True))')
    mc.button(l='REMOVE INBET', h=28, w=110, c='tools_facialShapes_v2.removeInbet(mc.ls(sl=True))')

    mc.setParent('..')
    mc.button(l='DUPPLCATE SHP', h=28, w=110, c='tools_facialShapes_v2.dupShp(mc.ls(sl=True))')
    #mc.button('switch_neutralTrgt', l='NEUTRAL/TRGT', h=28, w=110, c='tools_facialShapes_v2.showNeutral()')

    #mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='TRANSFER TO', h=28, w=110, c='tools_facialShapes_v2.snapShp(mc.ls(sl=True))')
    mc.button(l='RESET', h=28, w=110, c='tools_facialShapes_v2.resetShp(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='SET COR POSE', h=28, w=110, c='tools_facialShapes_v2.genCorSculpt(mc.ls(sl=True))')
    mc.button(l='SET INBET POSE', h=28, w=110, c='tools_facialShapes_v2.addInbet_v2(mc.ls(sl=True)[0], "COR")')
    mc.setParent('..')
    mc.button(l='CONNECT COR SCULPT', h=28, w=110, c='tools_facialShapes_v2.connectCorrective(mc.ls(sl=True))')
    mc.rowLayout(nc=2)
    mc.button(l='REMOVE COR', h=28, w=110, c='print "not available for the moment, sorryyyyy"')
    mc.button(l='REMOVE INBET', h=28, w=110, c='tools_facialShapes_v2.removeInbet(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='miror cor L to R', h=28, w=110, c='tools_facialShapes_v2.genFlipCor(mc.ls(sl=True), "L")')
    mc.button(l='miror cor R to L', h=28, w=110, c='tools_facialShapes_v2.genFlipCor(mc.ls(sl=True), "R")')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='gen mix sculpt', h=28, w=110, bgc=[.05,.2,.5], c='tools_facialShapes_v2.ctrMixCombination_v2(mc.ls(sl=True))')
    mc.button(l='connect mix shape', h=28, w=110, bgc=[.05,.05,.5], c='tools_facialShapes_v2.connectMixShp_v3(mc.ls(sl=True))')
    ...
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='select mix sculpt', h=28, w=110, c='tools_facialShapes_v2.genMixSculpt(mc.ls(sl=True))')
    mc.button(l='select mix trgt', h=28, w=110, c='tools_facialShapes_v2.connectMixShp(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=2)
    mc.button(l='load mix', h=28, w=110, c='tools_facialShapes_v2.loadMixValues(mc.ls(sl=True)[0])')
    mc.button(l='remove mix', h=28, w=110, c='tools_facialShapes_v2.connectMixShp(mc.ls(sl=True))')
    mc.setParent('..')
    mc.rowLayout(nc=1)
    mc.button(l='propagate mix to', h=28, w=110, c='tools_facialShapes_v2.propagateMix(mc.ls(sl=True))')
    ###
    mc.setParent('..')
    mc.setParent('..')
    mc.separator(h=10, st='in')
    ...
    mc.rowLayout(nc=2)
    mc.button(l='template node', h=28, w=110, c='tools_facialShapes_v2.convertToTemplate(mc.ls(sl=True))')
    mc.button(l='build rig', h=28, w=110, c='tools_facialShapes_v2.buildTpl()')
    mc.setParent('..')
    mc.button(l='update ctrl', h=28, w=110, c='tools_facialShapes_v2.majPosCtrl(mc.ls(sl=True))')
    ...
    mc.button(l='INIT SEP', h=28, w=110, c='tools_facialShapes_v2.genSepZones(mc.ls(sl=True)[0])')
    #mc.button(l='connect SEP', h=28, w=110, c='tools_facialShapes_v2.connectSep()')
    mc.button(l='UPDATE AUTO-WEIGHTS', h=28, w=110, c='tools_facialShapes_v2.majSepBaseWght()')
    ...
    #mc.button(l='ADD SEP', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.buildSep(mc.ls(sl=True), [mc.optionMenu("sepZone", q=True, v=True)], [mc.optionMenu("sepType", q=True, v=True)])')
    mc.button(l='ADD SLICE', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.buildSep(mc.ls(sl=True), ["None"], [mc.optionMenu("sepType", q=True, v=True)])')
    ...
    mc.button(l='COPY WGHT to', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.copyBsWghtTo(mc.ls(sl=True))')
    mc.button(l='REVERSE WGHT to (make negative of)', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.invertBsWghtTo_v2(mc.ls(sl=True))')

    mc.setParent('..')
    mc.rowLayout(nc=2, adj=1)
    mc.button(l='BUILD SYM MAP', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.crtSymAttr(mc.ls(sl=True))')
    mc.button(l='self FLIP WGHT', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.flipBsWght(mc.ls(sl=True))')

    mc.setParent('..')
    mc.rowLayout(nc=2, adj=1)
    mc.button(l='ADD WGHT TO', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.addBsWghtTo(lObj)')
    mc.button(l='Smab', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.mirrorBsWght(mc.ls(sl=True), "R")')

    mc.setParent('..')
    mc.rowLayout(nc=2, adj=1)
    mc.button(l='self MIRROR WGHT R <-- L', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.mirrorBsWght(mc.ls(sl=True), "L")')
    mc.button(l='self MIRROR WGHT R --> L', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.mirrorBsWght(mc.ls(sl=True), "R")')
    ...
    mc.rowLayout(nc=2, adj=1)
    mc.button(l='create sepCheaker', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.crtSepChecker(mc.ls(sl=True))')
    #mc.button(l='delete sepCheaker', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.crtSepChecker(mc.ls(sl=True))')
    ...
    mc.button(l='wrap to', h=28, w=110, c='tools_facialShapes_v2.connectNeutralWrap(mc.ls(sl=True))')
    ...
    mc.button(l='create Tpl', bgc=[.25,.2,.0], h=28, w=230, c='tools_facialShapes_v2.genTpl(mc.ls(sl=True))')
    mc.button(l='update Tpl', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.updateTpl(mc.ls(sl=True))')
    mc.button(l='report Tpl to neutral', bgc=[.25,.2,.5], h=28, w=230, c='tools_facialShapes_v2.connTplToNeutral()')
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=2)
    mc.columnLayout()
    mc.button(l='load as driver', c='tools_facialShapes_v2.loadSelItemInList("tplDriversList")')
    ...
    mc.setParent('..')
    mc.columnLayout()
    mc.checkBoxGrp('chkBoxTr', ncb=4, l='Translate : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v2.setchkBoxAllChans("chkBoxTr")')
    mc.checkBoxGrp('chkBoxRt', ncb=4, l='Rotate : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v2.setchkBoxAllChans("chkBoxRt")')
    mc.checkBoxGrp('chkBoxSc', ncb=4, l='Scale : ', la4=['X', 'Y', 'Z', 'All'], ct5=('left', 'left', 'left', 'left', 'left'), co5=(0, -75, -150, -225, -290), cc4='tools_facialShapes_v2.setchkBoxAllChans("chkBoxSc")')
    mc.setParent('..')
    mc.columnLayout()
    mc.button(l='load as driven', c='tools_facialShapes_v2.loadSelItemInList("tplDrivensList")')
    ...
    mc.setParent('..')
    mc.setParent('..')
    mc.button(l='APPLY', bgc=[.25,.2,.5], c='tools_facialShapes_v2.linkSlicesDrivers()')
    ...
    mc.rowLayout(nc=1, adj=1)
    mc.button(l='EXTRACT SHAPES', bgc=[.2,.2,.5], h=28, w=230, c='tools_facialShapes_v2.extractShapes_v3(mc.ls(sl=True))')


    mc.setParent('..')
    mc.rowLayout(nc=1, adj=1)
    mc.button(l='CONNECT SHAPES', bgc=[.2,.2,.5], h=28, w=230, c='tools_facialShapes_v2.buildShapesTRGTRig(mc.ls(sl=True))')
    ...
    mc.rowLayout(nc=3, adj=2)
    mc.button(l='template face', h=28, w=110, c='tools_facialShapes_v2.setFaceTpl()')
    mc.button(l='build face', h=28, w=110, c='tools_facialShapes_v2.buildFaceTpl()')
    mc.button(l='connect face', h=28, w=110, c='tools_facialShapes_v2.connectFaceRig_v2(mc.ls(sl=True)[0])')
