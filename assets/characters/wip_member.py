...
class Member(rig_world.RigWorld):
    ...
    def createMember(self):
        ...
        for i, eachTpl in enumerate(lsInfo):
            ...
            if(self.IkType != 'ikSplineSolver'):
                ...
                mc.poleVectorConstraint(lsPlVCtr[0], ikHandle)
            ...
            if int(numbPart) >1:
                ...
                if int(numbPart) %2 ==0:
                    ...
                    nRigAnkleRot = gloss.name_format(prefix=gloss.lexicon('rig'),name=nWrist,nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
                    nCnsAnkleRot = gloss.name_format(prefix=gloss.lexicon('cns'),name=nWrist,nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
                    nAimAnkleRot = gloss.name_format(prefix=gloss.lexicon('aim'),name=nWrist,nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
                    nUpVAnkleRot = gloss.name_format(prefix=gloss.lexicon('upV'),name=nWrist,nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
                    nAnkleRot = gloss.name_format(prefix=gloss.lexicon('c'),name=nWrist,nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
                    nAnkleRotDistD = gloss.name_format(prefix=gloss.lexicon('rot'),name=nWrist,nFunc='dstD', nSide=side,incP=incPart)

                    rigAnkleRot = libRig.createObj(partial(mc.group, **{'n': nRigAnkleRot, 'em': True}),match=[ikCtr['c']],father=nConsSpine)
                    cnsAnkleRot = libRig.createObj(partial(mc.group, **{'n': nCnsAnkleRot, 'em': True}),match=[ikCtr['c']],father=rigAnkleRot)
                    aimAnkleRot = libRig.createObj(partial(mc.group, **{'n': nAimAnkleRot, 'em': True}),match=[ikCtr['c']],father=cnsAnkleRot)
                    upVAnkleRot = libRig.createObj(partial(mc.group, **{'n': nUpVAnkleRot, 'em': True}),match=[ikCtr['c']],father=cnsAnkleRot)

                    mc.move(mvUpV[0],mvUpV[1],mvUpV[2],upVAnkleRot, ls=True)
                    mc.pointConstraint(ikCtr['c'],cnsAnkleRot)
                    aimRot= (0.0,1.0,0.0)
                    upVRot= (1.0,0.0,0.0)
                    mc.aimConstraint(lsChainFk[0], aimAnkleRot, aim=aimRot, u=upVRot, wut='object', wuo=upVAnkleRot)
                    ankleRot = libRig.createController(name=nAnkleRot,shape=libRig.Shp([self.typeCircle],color=valColorCtrIK,
                               size=(0.5*sclWorld,0.5*sclWorld,0.5*sclWorld),rotShp=(0,0,90)),match=[cnsAnkleRot], father=aimAnkleRot,attributs={"drawStyle":2})
                    ankleRotDistD = libRig.createObj(partial(mc.group, **{'n': nAnkleRotDistD, 'em': True}),match=[ikCtr['c']],father=ankleRot['c'])
                    mc.delete(mc.listRelatives(lsPlVRoot[-1], type="parentConstraint"))
                    mc.parent(lsPlVRoot[-1],ankleRotDistD)

                    # CREATE ATTRIBUTE QUADLOCK________________________
                    mc.addAttr(ikCtr['c'], ln="quadLock", nn="QuadLock",at="double", min=-10, max=10, dv=0)
                    mc.setAttr(ikCtr['c'] + ".quadLock", e=True, k=True)

                    # node to drive ankleRotDistD_____________
                    nAddDLinAnkleRot= gloss.name_format(prefix=gloss.lexicon('addDblLin'),name=nWrist, nFunc=gloss.lexicon('rot'),nSide=side,incP=incPart)
                    nodeAddDLinAnkleRot = mc.shadingNode("addDoubleLinear",n=nAddDLinAnkleRot,asUtility=True)
                    mc.setAttr("%s.input2" % (nodeAddDLinAnkleRot), -(mc.getAttr(nNodeDstDim +'.distance')))
                    mc.connectAttr("%s.distance" % (nNodeDstDim), "%s.input1" % (nodeAddDLinAnkleRot), force=True)
                    nMltDivAnkleRot2 = gloss.name_format(prefix=gloss.lexicon('mltDiv'), name=nWrist, nFunc=gloss.lexicon('rot')+'Multi', nSide=side,incP=incPart)
                    mltDivAnkle2 = mc.createNode("multiplyDivide", n=nMltDivAnkleRot2)
                    mc.setAttr(mltDivAnkle2 + ".operation", 2)
                    mc.setAttr(mltDivAnkle2 + ".input2X",mc.getAttr(nNodeDstDim +'.distance'))
                    mc.connectAttr("%s.output" % (nodeAddDLinAnkleRot), "%s.input1X" % (mltDivAnkle2), force=True)

                    nMltDivAnkleRot = gloss.name_format(prefix=gloss.lexicon('mltDiv'), name=nWrist, nFunc=gloss.lexicon('rot')+'Div', nSide=side,incP=incPart)
                    ...
                    mc.connectAttr("%s.quadLock" % (ikCtr['c']), "%s.input1X" % (mltDivAnkle), force=True)
                    nMltDivAnkleRot3 = gloss.name_format(prefix=gloss.lexicon('mltDiv'), name=nWrist, nFunc=gloss.lexicon('rot')+'Div2', nSide=side,incP=incPart)
                    ...
                    nMltDivAnkleRot4 = gloss.name_format(prefix=gloss.lexicon('mltDiv'), name=nWrist, nFunc=gloss.lexicon('rot'), nSide=side,incP=incPart)
            ...
            for z, each in enumerate(lsChainFk[1:len(concatIkFk)-(lenLsTplMidMb-1)]):
                ...
                stretchPart = "+(%s/%s)"%(ikCtr['c'] + ".stretchPart%s"%countExp,10)
            ...
            mc.setAttr(condLock + ".colorIfFalseR",2)
            mc.setAttr(condLock + ".operation",3)
            mc.setAttr(condLock + ".secondTerm",0.1)
            ...
            #mc.parent(lsJntLoftBase[0],aimTwistTop)
            ...
            mc.connectAttr("%s.output" % (NodeMultDblLinear), "%s.rotateY" % (lsJntLoftBase[0]), force=True)
            '''
            ...
            twistHdl2 = libRig.createObj(partial(mc.joint, **{'n': nTwistHdl2}), match=[nJntFirst], father=twistHdl1,
                             attributs={"jointOrientX": 0, "jointOrientY": 0, "jointOrientZ": 0, "drawStyle": 2})
            mc.setAttr(twistHdl1+'.rotateOrder',lsRotOrder[0])
            mc.setAttr(twistHdl2+'.rotateOrder',lsRotOrder[0])
            ...
            if self.nameEnd == 'hand':
                ...
                mc.move(1*mc.getAttr("tpl_WORLD"+".scaleX"),0,0,twistHdl2, ls=True)
                aim= (-1.0,0.0,0.0)
                upV= (0.0,0.0,1.0)
                if side =='R':
                    aim= (1.0,0.0,0.0)
            ...
            ikHdlTwistHdl =libRig.createObj(partial(mc.ikHandle, **{'n':nTwistIkHdl ,'sj':twistHdl1,'ee':twistHdl2,'sol':self.IkType}),
                        match=[twistHdl2],father=nJntFirst,attributs={"rotateX":0,"rotateY":0,"rotateZ":0,"snapEnable": 0,"visibility":0})
    ...
    def createMidMember(self):
        ...
        for i, eachEl in enumerate(sorted(dic)):
            ...
            nMltDivRollBreak1 = gloss.name_format(prefix=gloss.lexicon('mltDiv'),name=self.nameEnd,nFunc=gloss.lexicon('roll')+'BreakDiv',nSide=side,incP=incPart)
            ...
            multDivRollBreak1 = mc.createNode("multiplyDivide", n=nMltDivRollBreak1)
            mc.setAttr(multDivRollBreak1 + ".operation", 2)
            mc.setAttr(multDivRollBreak1 + ".input2X",10)
            mc.connectAttr(dic[eachEl]['ik'] + '.rollBreak', "%s.input1X" % (multDivRollBreak1), force=True)
            multDivRollBreak2 = mc.createNode("multiplyDivide", n=nMltDivRollBreak2)
            mc.setAttr(multDivRollBreak2  + ".operation", 1)
            mc.setAttr(multDivRollBreak2  + ".input2X",90)
            mc.connectAttr(multDivRollBreak1 + '.outputX', "%s.input1X" % (multDivRollBreak2), force=True)
            ...
            if mc.getAttr(eachEl+'.endMb[0]') != "0":
                ...
                for key,values in sorted(dicTplEndMb.items()):
                    ...
                    for i, each in enumerate(values):
                        ...
                        mc.connectAttr(endCtr['c'] + '.rotateZ', "%s.input1" % (NodeMltDblLinSkAdd), force=True)
                        ...
                        mc.connectAttr(mltDivSkAdd + ".outputX", "%s.rotateZ" % (bufSkAdd), force=True)
                        ...
                        mc.connectAttr(endCtr['c'] + '.rotateZ', "%s.input1" % (NodeMltDblLinScl), force=True)
                        ...
                        mc.connectAttr(endCtr['c'] + '.rotateZ', cndScl + '.firstTerm')
                        mc.connectAttr(addDblLScl + '.output', cndScl + '.colorIfTrueR')
                        mc.connectAttr(cndScl + ".outColorR", "%s.scaleX" % (bufSkAdd), force=True)

                    # ADD ATTRIBUTES ROTATION________________________________
                    for i, each in enumerate(lsEndMbCtr):
                        ...
                        # CONNECT ROTATE AXIS END MEMBEB_________________________
                        mc.connectAttr(lsEndMbCtr[0] + ".%sRoll%s"%(part,i+1), "%s.rotateAxisZ" % (each), force=True)
