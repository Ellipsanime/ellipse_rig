...
class Spine(rig_world.RigWorld):

    def __init__(self, name='spine', tplInfo='tplInfo', hook = '',*args):
    ...
    def createSpine(self):
        ...
        for i, eachTpl in enumerate([self.info]):
            ...
            nCog = gloss.name_format(prefix=gloss.lexicon('c'),name=gloss.lexicon('cog'),nSide=side,incP=incPart)
            nDownIk = gloss.name_format(prefix=gloss.lexicon('c'),name=gloss.lexicon('pelvis'),nFunc=gloss.lexicon('ik'),nSide=side,incP=incPart)
            nTopIk = gloss.name_format(prefix=gloss.lexicon('c'),name=self.name,nFunc=gloss.lexicon('ik'),nSide=side,incP=incPart)
            ...
            hookSpine = libRig.createObj(partial(mc.group, **{'n': nHook, 'em': True}), Match=self.nFly,father=self.nRig)
            # CONNECT LOC_FLY WITH HOOK SPINE_____________________________________
            '''
            ...
            mc.connectAttr((dMatrixCog + ".outputRotate"), (hookSpine+ ".rotate"))
            mc.connectAttr((dMatrixCog + ".outputScale"), (hookSpine + ".scale"))
            mc.connectAttr((dMatrixCog + ".outputShear"), (hookSpine+ ".shear"))
            mc.connectAttr((dMatrixCog + ".outputTranslate"), (hookSpine+ ".translate"))
            mc.select(cl=True)
            '''
            ...
            mc.parent(GrpNoTouch, hookSpine)
            # COG___________________________________________________________________
            ctrCog = libRig.createController(name=nCog,shape=libRig.Shp([self.typeCircle],color=valColorMstCtr,size=(1.5,1.5,1.5)),match=master,father=hookSpine)
            ...
            if len(getPtOnCv) < 2:
                ...
                for i, eachTrf in enumerate(getTrf):
                    ...
                    nCtrlShp = gloss.name_format(prefix=gloss.lexicon('c'),name=self.name+str(i+1)+'Shp', nSide=side,incP=incPart)
                    ...
                    mc.select(cl=True)
                    ctrlShp = mc.joint(n=nCtrlShp, p=positions2, o=(rotations2[0], rotations2[1], rotations2[2]))
                    mc.setAttr(getJts +".drawStyle",2)
                    mc.select(cl=True)
                ...
                [mc.parent(each,lCtrlShp[i])for i, each in enumerate(lCtrlShp[1:])]
                ...
                for each in lCtrlShp[:-1]: mc.joint(each, e=True, oj="yxz", sao="xup")
                ...
                mc.setAttr(lCtrlShp[-1] + ".jointOrientX", 0)
                mc.setAttr(lCtrlShp[-1] + ".jointOrientY", 0)
                mc.setAttr(lCtrlShp[-1] + ".jointOrientZ", 0)
                ...
                [mc.parent(ctrlShp, lsBufSk[i]) for i, ctrlShp in enumerate(lCtrlShp)]
                [mc.parent(eachJtSk, lCtrlShp[i]) for i, eachJtSk in enumerate(lsSk)]
                [mc.parent(jtAnchor, lCtrlShp[i]) for i, jtAnchor in enumerate(lsAnchor)]
                ...
                #mc.connectAttr(hookSpine + '.scale', lsSk[-1] + '.scale')

                # SYSTEM CREATE IK HANDLE__________________________________________
                mc.orientConstraint(topIK["c"],ojt)
                '''
                ...
                mc.select(cl=True)
                getJtGrp = libRig.createObj(partial(mc.joint, **{'n':nGrpJts}),match=lsTplIk[0], father=ctrCog["c"],attributs={"drawStyle": 2,"visibility": 1})
                ...
                if len(concatIkFk) > 2:
                    ...
                    # CREATE ADDITIONAL FK IN PELVIS_________________________________
                    # NAME___________________________________________________________
                    nAddFk = gloss.name_format(prefix=gloss.lexicon('c'),name=self.name,nFunc=gloss.lexicon('dn'), nSide=side,incP=incPart)
                    # CREATE_________________________________________________________
                    sclW = mc.getAttr(('tpl_'+ self.nWorld) +'.scale')[0]
                    createAddFk = libRig.createController(name=nAddFk,shape=libRig.Shp([self.typeCircle],color=valColorCtr,size=(1*sclW[0],1*sclW[0],1*sclW[0])),match=[downIK['c']])
                    mc.setAttr(createAddFk["root"] + ".rotateOrder",libRgPreset.configAxis(mb="rOrdSpine",side=side)["rOrdFk"], l=False, k=True)
                    mc.setAttr(createAddFk["c"] + ".rotateOrder",libRgPreset.configAxis(mb="rOrdSpine",side=side)["rOrdFk"], l=False, k=True)
                    mc.parent(nSwitchsCtr, createAddFk["c"], s=True, add=True)
                    lCtrl.append(createAddFk["c"])
                    # CREATE CLUSTER_________________
                    nClusterDnFk = gloss.name_format(prefix=gloss.lexicon('cls'), name=self.name,nFunc=gloss.lexicon('dn'), nSide=side, incP=incPart)
                    clusterDnFk = mc.createNode("cluster", n=nClusterDnFk)
                    nMltMatCluster = gloss.name_format(prefix=gloss.lexicon('mltM'), name=self.name,nFunc=gloss.lexicon('dn'), nSide=side, incP=incPart)
                    mltMatCluster = mc.createNode("multMatrix", n=nMltMatCluster)
                    mc.connectAttr(createAddFk['c'] + '.worldMatrix[0]', mltMatCluster + '.matrixIn[0]')
                    mc.connectAttr(createAddFk['root'] + '.worldInverseMatrix[0]', mltMatCluster + '.matrixIn[1]')
                    mc.connectAttr(mltMatCluster + '.matrixSum', clusterDnFk + '.weightedMatrix')
                    mc.connectAttr(createAddFk['c'] + '.worldMatrix[0]', clusterDnFk + '.matrix')
                    mc.connectAttr(createAddFk['root'] + '.worldInverseMatrix[0]', clusterDnFk + '.bindPreMatrix')
                    mc.connectAttr(createAddFk['root'] + '.worldMatrix[0]', clusterDnFk + '.preMatrix')
                    # PARENT_________________________________________________________
                    mc.parent(createAddFk["root"], downIK["c"])
                    # PARENT CONSTRAINT FIRST SK TO nADDFK___________________________
                    #mc.parentConstraint(createAddFk["c"], lsSk[0])
                else:
                    pass
                ...
                lSa = lib_connectNodes.surfAttach(selObj=[nLoftBase], lNumb=1,parentInSA=True, parentSA=self.nSa,delKnot=True)
                valmaxU = (mc.getAttr(nLoftBase + ".minMaxRangeU"))[0][1]
                mc.setAttr(lSa['loc'][0] + ".U",(valmaxU/2))
                mc.parent(cv1,self.nWorldCenter)
                mc.parent(cv2,self.nWorldCenter)

                nIkMiddle = gloss.name_format(prefix=gloss.lexicon('c'),name=self.name,nFunc=gloss.lexicon('mid'), nSide=side,incP=incPart)
                ikMiddle = libRig.createController(name=nIkMiddle,shape=libRig.Shp([self.typeCircle],color=valColorCtrIK,size=(1,1,1)),
                                                   father=lsFkControl[len(lsFkControl)/2],match=[lsFkControl[len(lsFkControl)/2]])
                ...
                # CREATE CLUSTER_________________
                nCluster = gloss.name_format(prefix=gloss.lexicon('cls'), name=self.name, nFunc=gloss.lexicon('mid'),nSide=side,incP=incPart)
                clusterIkMid = mc.createNode("cluster", n=nCluster)
                nMltMatCluster = gloss.name_format(prefix=gloss.lexicon('mltM'),name=self.name, nFunc=gloss.lexicon('mid'),nSide=side,incP=incPart)
                mltMatCluster = mc.createNode("multMatrix", n=nMltMatCluster)
                mc.connectAttr(ikMiddle['c'] + '.worldMatrix[0]', mltMatCluster + '.matrixIn[0]')
                mc.connectAttr(ikMiddle['root'] + '.worldInverseMatrix[0]', mltMatCluster + '.matrixIn[1]')
                mc.connectAttr(mltMatCluster + '.matrixSum', clusterIkMid + '.weightedMatrix')
                mc.connectAttr(ikMiddle['c'] + '.worldMatrix[0]', clusterIkMid + '.matrix')
                mc.connectAttr(ikMiddle['root'] + '.worldInverseMatrix[0]', clusterIkMid + '.bindPreMatrix')
                mc.connectAttr(ikMiddle['root'] + '.worldMatrix[0]', clusterIkMid + '.preMatrix')
                # ADD SWITCH TO ADD IK CONTROL MIDDLE_________________________________
                mc.parent(nSwitchsCtr, ikMiddle["c"], s=True, add=True)
                lsClusters = [clusterIkDwn,clusterIkMid,clusterIkTop]

                '''
                #######
                nLoftBasetest = "test"
                loftBasetest = lib_curves.createLoft(name=nLoftBasetest, objLst=[cv1,cv2], father=None, deleteCrv=False)
                print  lsFkControlAll

                # concat points cv by 4 _______________________________
                recLoft = mc.ls(loftBasetest + '.cv[*]', fl=True)
                adjustNumb = 4
                selDiv2 = int(math.ceil(float(len(recLoft)) / adjustNumb))
                val = 0
                lsPoint = []
                for each2 in range(selDiv2):
                    part = []
                    for each in range(adjustNumb):
                        part.append(recLoft[each + val])
                    val += adjustNumb
                    lsPoint.append(part)
                ...
                count = 0
                getVal = []
                for each in range(len(lsPoint)):
                    val = round(abs(float(count)/float(len(lsPoint) -1)), 4)
                    count += 1
                    getVal.append(val)
                invertVal = getVal[::-1]

                # Skin loft
                skinLoftTest = mc.skinCluster(lsFkControlAll, loftBasetest, tsb=1, bm=1, nw=1, mi=3, omi=True, dr=4, rui=True)
                ...
                # adjust value cluster DnFK
                #mc.deformer(nClusterDnFk, e=True, g=recCv[0])
                #mc.deformer(nClusterDnFk, e=True, g=recCv[1])
                ...
                # adjust value cluster Middle loft
                ...
                mc.deformer(lsClusters[2], e=True, g=recCv[0])
                mc.deformer(lsClusters[2], e=True, g=recCv[-1])
                mc.percent(lsClusters[2], recCv[0], value=0)

                mc.deformer(lsClusters[2], e=True, g=recCv1[0])
                mc.deformer(lsClusters[2], e=True, g=recCv1[-1])
                mc.percent(lsClusters[2], recCv1[0], value=0)

                mc.deformer(lsClusters[2], e=True, g=recCv2[0])
                mc.deformer(lsClusters[2], e=True, g=recCv2[-1])
                mc.percent(lsClusters[2], recCv2[0], value=0)

                for i, each in enumerate(recCv[1:-1]):
                    mc.deformer(lsClusters[2], e=True, g=each)
                    mc.percent(lsClusters[2], each, value=val[i])
                ...
                for i, each in enumerate(recCv2[1:-1]):
                    mc.deformer(lsClusters[2], e=True, g=each)
                    mc.percent(lsClusters[2], each, value=val[i])
                # MIDDLE CLUSTER VALUE________________________
                lenCv = len(recCv)
                getPart1 = []
                getPart2 = []
                for i, each in enumerate(recCv):
                    if i <= lenCv/2:
                        getPart1.append(each)
                for i, each in enumerate(recCv):
                    if lenCv %2:
                        if i >= lenCv/2:
                            getPart2.append(each)
                    else:
                        if i >= (lenCv/2)-1:
                            getPart2.append(each)

                val = [round(abs(-i/float((lenCv/2))), 4) for i, each in enumerate(getPart1)]
                for i, each in enumerate(getPart1):
                    mc.deformer(lsClusters[1], e=True, g=each)
                    mc.percent(lsClusters[1], each, value=val[i])
                for i, each in enumerate(getPart2):
                    mc.deformer(lsClusters[1], e=True, g=each)
                    mc.percent(lsClusters[1], each, value=val[::-1][i])
                mc.percent(lsClusters[1], recCv[1], value=0)
                mc.percent(lsClusters[1], recCv[-2], value=0)
                ...
                valAttr = 10
                expStretch = lib_connectNodes.expStretchSpine(self.name,incPart, ctrCog['c'], nArclen, cvLen, lsSk,valAttr, numbSk, nSwitchsCtr,lsBufSk, getDistDim,valDist,side)
                ...
                mc.sets(createAddFk['sk'], edit=True, forceElement=nSetPart)
                ...
                lsHooks = lsAnchor[1:]
                lsHooks.insert(0, createAddFk["sk"])
                ...
                lsFkCtr = lsFkControl
                #lsFkCtr.insert(0, createAddFk["c"])
                for node in lCtrlShp[:-1]:
                    mc.setAttr(getJts+'.displayHandle', 1)

                lCtrlShp.insert(0, createAddFk["c"])
                lCtrlShp.insert(1, ikMiddle['c'])
                for node in lCtrlShp:
                    mc.setAttr(node+'.overrideEnabled', 1)
                    mc.setAttr(node+'.overrideColor', 18)
                ...
                if mc.objExists(eachTpl+'.%s'%cgMaster):
                    ...
                    mc.addAttr(eachTpl, ln=cgFkShp,dt='string',multi=True) # add Cg fkShp
                    ...
                    [mc.setAttr(eachTpl+'.%s['%(cgFkShp) +str(i)+']',each,type='string') for i, each in enumerate(lCtrlShp)]

                # PARENT HOOK WITH RIG MEMBER________________________________________________
                mc.parent(hookSpine,self.hook)
